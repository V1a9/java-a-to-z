...
<head>
    ...
    <title>Add User - ${loggedUser.name} (${loggedUserRole})</title>
</head>
<body>
...
<div class="container">
    <h5>Add User:</h5>
    <form method="post" class="form-horizontal" id="user" action="${pageContext.servletContext.contextPath}/testexercise/adduser">
        ...
        <div class="container">
            <label class="control-label col-sm-2">Music:</label>
        </div>
...
<script>
    $(document).ready(function () {
        $.getJSON("../testexercise/entities", {entity: "music"}, function (json) {
            var musicIndex = 0;
            $.each(json, function (index, element) {
                var div = $("<div/>", {"class":"form-check form-check-inline"});
                var label = $("<label/>", {"class":"form-check-label"});
                var input = $("<input/>", {type:"checkbox", "class":"form-check-input", name:"music" + musicIndex++, value:element});

                input.appendTo(label);
                div.append(label);
                div.append(element);
                div.appendTo($("div.container:eq(1)"));
            });
